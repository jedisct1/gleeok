...
def ip64_128(msg, key, hk0, hk1, nonce):
    ...
    c = xor128(aes128(nonce + int2bytes(0x1, 32), key), m[0])
    ...
    tag = xor128(aes128(nonce + int2bytes(0x0, 32), key), s + t)
...
def ip64_256(msg, key, hk0, hk1, nonce):
    ...
    c0 = xor128(aes128(nonce + int2bytes(0x1, 32), key), m[0])
    c1 = xor128(aes128(nonce + int2bytes(0x2, 32), key), m[1])
...
def ip64_512(msg, key, hk0, hk1, nonce):
    ...
    c0 = xor128(aes128(nonce + int2bytes(0x1, 32), key), m[0])
    c1 = xor128(aes128(nonce + int2bytes(0x2, 32), key), m[1])
    c2 = xor128(aes128(nonce + int2bytes(0x3, 32), key), m[2])
    c3 = xor128(aes128(nonce + int2bytes(0x4, 32), key), m[3])
...
for i in range(100):
    ...
    key   = int2bytes(random.randint(0, 2**128-1), 128)
    hk0   = int2bytes(random.randint(0, 2**256-1), 512)
    hk1   = int2bytes(random.randint(0, 2**256-1), 512)
    msg   = int2bytes(random.randint(0, 2**256-1), 512)
